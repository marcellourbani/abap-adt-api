...
import { HttpResponse, session_types } from ".";
...
const convertResponse = (original?: HttpClientResponse): ResponseData => {
    if (!original) return { headers: {}, statusCode: 0, statusMessage: "" }
...
export const logResponse = (clientId: number, original: HttpClientResponse, config: HttpClientOptions, callback: LogCallback | undefined) => {
    try {
        if (!callback) return
